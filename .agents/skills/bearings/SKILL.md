...
3. **Write the dated report file so it persists, then surface the mandatory four-section digest in chat.**
   ...
   - For a richer review surface, optionally create a Lavish board with `lavish-axi` and deliver its finalized HTML through `bin/fm-html-review.sh`, but the markdown file is the required artifact and the four-section chat digest is the required minimum.

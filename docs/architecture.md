...
A declared pause also supersedes a matching terminal failed or cancelled run when it is proven to have been declared after that run finished, so a crew that recovered from a failed validation and now waits on an external dependency reports `paused` rather than resurfacing `failed`; active, parked, and successful runs and any unprovable timing stay authoritative, and the script header owns that precedence contract.
